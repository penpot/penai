...
class HierarchyElement:
    ...
    def from_hierarchy_schema(
        ...
        for child in source_element.get("children", []):
...
class SchemaResponse(Response):
    def __init__(self, response_text: str, parser: BaseOutputParser):
...
class HierarchyInferencer:
    ...
    def __init__(self, model: RegisteredLLM = RegisteredLLM.GPT4O, validate_hierarchy: bool = True):
        self.model = model
        self.validate_hierarchy = validate_hierarchy

    def build_prompt(self, visualizations: list[ShapeVisualization]):
        query = (
    ...
    def _validate_inferred_hierarchy(self, hierarchy: HierarchyInferenceElement):
        map(attrgetter("id"), hierarchy.traverse())

    def infer_shape_hierarchy(self, shape: PenpotShapeElement, return_visualizations: bool = False):
